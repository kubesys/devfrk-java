...
package io.github.kubesys.devfrk.httpserver.impl;
...
import io.github.kubesys.devfrk.httpserver.HttpsConfigurator;
import io.github.kubesys.devfrk.httpserver.HttpsParameters;
...
/**
 * given a non-blocking SocketChannel, it produces (blocking) streams which
 * encrypt/decrypt the SSL content and handle the SSL handshaking automatically.
 */
...
//END_TIGER_EXCLUDE
	...
	/**
	 * request the engine to repeat the handshake on this session the handshake must
	 * be driven by reads/writes on the streams Normally, not necessary to call
	 * this.
	 */
	...
	class WrapperResult {
		...
		/*
		 * if passed in buffer was not big enough then the a reallocated buffer is
		 * returned here
		 */
		ByteBuffer buf;
	}
	...
	/*
	 * reallocates the buffer by :- 1. creating a new buffer double the size of the
	 * old one 2. putting the contents of the old buffer into the new one 3. set
	 * xx_buf_size to the new size if it was smaller than new size
	 *
	 * flip is set to true if the old buffer needs to be flipped before it is
	 * copied.
	 */
	...
	/**
	 * This is a thin wrapper over SSLEngine and the SocketChannel, which guarantees
	 * the ordering of wraps/unwraps with respect to the underlying channel
	 * read/writes. It handles the UNDER/OVERFLOW status codes It does not handle
	 * the handshaking status codes, or the CLOSED status code though once the
	 * engine is closed, any attempt to read/write to it will get an exception. The
	 * overall result is returned. It functions synchronously/blocking
	 */
	class EngineWrapper {
		...
		/*
		 * try to wrap and send the data in src. Handles OVERFLOW. Might block if there
		 * is an outbound blockage or if another thread is calling wrap(). Also, might
		 * not send any data if an unwrap is needed.
		 */
		...
		/*
		 * block until a complete message is available and return it in dst, together
		 * with the Result. dst may have been re-allocated so caller should check the
		 * returned value in Result If handshaking is in progress then, possibly no data
		 * is returned
		 */
				...
				do {
					...
					if (status == Status.BUFFER_UNDERFLOW) {
						if (unwrap_src.limit() == unwrap_src.capacity()) {
							...
							/*
							 * Buffer not full, just need to read more data off the channel. Reset pointers
	...
	/**
	 * send the data in the given ByteBuffer. If a handshake is needed then this is
	 * handled within this method. When this call returns, all of the given user
	 * data has been sent and any handshake has been completed. Caller should check
	 * if engine has been closed.
	 */
	...
	/**
	 * read data thru the engine into the given ByteBuffer. If the given buffer was
	 * not large enough, a new one is allocated and returned. This call handles
	 * handshaking automatically. Caller should check if engine has been closed.
	 */
	...
	/*
	 * we've received a close notify. Need to call wrap to send the response
	 */
	...
	/*
	 * do the (complete) handshake after acquiring the handshake lock. If two
	 * threads call this at the same time, then we depend on the wrapper methods
	 * being idempotent. eg. if wrapAndSend() is called with no data to send then
			...
			while (hs_status != HandshakeStatus.FINISHED && hs_status != HandshakeStatus.NOT_HANDSHAKING) {
				...
				case NEED_TASK:
					...
					while ((task = engine.getDelegatedTask()) != null) {
						/*
						 * run in current thread, because we are already running an external Executor
						 */
						task.run();
					}
	...
	/**
	 * represents an SSL input stream. Multiple https requests can be sent over one
	 * stream. closing this stream causes an SSL close input.
	 */
	class InputStream extends java.io.InputStream {
		...
		/**
		 * close the SSL connection. All data must have been consumed before this is
		 * called. Otherwise an exception will be thrown. [Note. May need to revisit
		 * this. not quite the normal close() symantics
		 */
	...
	/**
	 * represents an SSL output stream. plain text data written to this stream is
	 * encrypted by the stream. Multiple HTTPS responses can be sent on one stream.
	 * closing this stream initiates an SSL closure
	 */
