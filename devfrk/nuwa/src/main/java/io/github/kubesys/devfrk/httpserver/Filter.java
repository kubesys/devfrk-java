...
package io.github.kubesys.devfrk.httpserver;
...
public abstract class Filter {
	...
	/**
	 * a chain of filters associated with a HttpServer. Each filter in the chain is
	 * given one of these so it can invoke the next filter in the chain
	 */
	public static class Chain {
		...
		/**
		 * calls the next filter in the chain, or else the users exchange handler, if
		 * this is the final filter in the chain. The Filter may decide to terminate the
		 * chain, by not calling this method. In this case, the filter <b>must</b> send
		 * the response to the request, because the application's exchange handler will
		 * not be invoked.
		 * 
	 ...
	 * <li>filter the request body or the response body, by creating suitable filter
	 * streams and calling
	 ...
	 * <li>if option 2. above taken, then this Filter must use the HttpExchange to
	 * send back an appropriate response</li>
	 * </ul>
	 * <p>
	 * 
	 ...
	 * @throws NullPointerException if either exchange or chain are
	 *                              <code>null</code>
	 */
