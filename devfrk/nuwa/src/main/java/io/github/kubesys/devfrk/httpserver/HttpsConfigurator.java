 ...
 * c.getDefaultSSLParameters(); if (remote.equals (...) ) { // modify the
 * default set for client x }
 *
...
//BEGIN_TIGER_EXCLUDE
	/**
	 * Called by the HttpsServer to configure the parameters for a https connection
	 * currently being established. The implementation of configure() must call
	 * {@link HttpsParameters#setSSLParameters(SSLParameters)} in order to set the
	 * SSL parameters for the connection.
	 * <p>
	 * The default implementation of this method uses the SSLParameters returned
	 * from
	 * <p>
	 * <code>getSSLContext().getDefaultSSLParameters()</code>
	 * <p>
	 * configure() may be overridden in order to modify this behavior. See, the
