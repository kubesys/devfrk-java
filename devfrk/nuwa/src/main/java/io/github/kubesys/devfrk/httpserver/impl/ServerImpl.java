...
import io.github.kubesys.devfrk.HttpHandler;
import io.github.kubesys.devfrk.HttpServer;
import io.github.kubesys.devfrk.httpserver.Filter;
import io.github.kubesys.devfrk.httpserver.Headers;
import io.github.kubesys.devfrk.httpserver.HttpExchange;
import io.github.kubesys.devfrk.httpserver.HttpsConfigurator;
import io.github.kubesys.devfrk.httpserver.impl.HttpConnection.State;
...
class ServerImpl {
	 ...
	 * m_logger
	 */
	public static Logger m_logger = Logger.getLogger(ServerImpl.class.getName());
	...
	/*
	 * following two are used to keep track of the times when a connection/request
	 * is first received and when we start to send the response
	 */
	...
	ServerImpl(HttpServer wrapper, String protocol, InetSocketAddress addr, int backlog) throws IOException {
		...
		if (timer1Enabled) {
			...
			m_logger.info("HttpServer timer1 enabled period in ms:  " + TIMER_MILLIS);
			m_logger.info("MAX_REQ_TIME:  " + MAX_REQ_TIME);
			m_logger.info("MAX_RSP_TIME:  " + MAX_RSP_TIME);
		}
		events = new LinkedList<Event>();
		m_logger.info("HttpServer created " + protocol + " " + addr);
	}
	...
	public synchronized HttpContextImpl createContext(String path, HttpHandler handler) {
		...
		m_logger.info("context created: " + path);
		return context;
	}
	...
	public synchronized void removeContext(String path) throws IllegalArgumentException {
		...
		m_logger.info("context removed: " + path);
	}
	...
	public synchronized void removeContext(HttpContext context) throws IllegalArgumentException {
		...
		m_logger.info("context removed: " + context.getPath());
	}
	...
	class Dispatcher implements Runnable {
		...
		private void handleEvent(Event r) {
			...
			try {
				...
				m_logger.severe("Dispatcher (1)" + e);
				c.close();
			}
		}
		...
		void reRegister(HttpConnection c) {
			...
			try {
				...
				dprint(e);
				m_logger.severe("Dispatcher(8)" + e);
				c.close();
			}
		}
		...
		public void run() {
			while (!finished) {
				try {
					...
					m_logger.log(Level.FINER, "Dispatcher (4)", e);
				} catch (Exception e) {
					m_logger.log(Level.FINER, "Dispatcher (7)", e);
				}
			}
			try {
				selector.close();
		...
		private void handleException(SelectionKey key, Exception e) {
			HttpConnection conn = (HttpConnection) key.attachment();
			if (e != null) {
				m_logger.log(Level.FINER, "Dispatcher (2)", e);
			}
		...
		public void handle(SocketChannel chan, HttpConnection conn) throws IOException {
			try {
				...
				m_logger.log(Level.FINER, "Dispatcher (4)", e1);
				closeConnection(conn);
			} catch (IOException e) {
				m_logger.log(Level.FINER, "Dispatcher (5)", e);
				closeConnection(conn);
			}
		}
	}
	...
	class Exchange implements Runnable {
		...
		public void run() {
			...
			try {
				if (context != null) {
					...
					if (https) {
						if (sslContext == null) {
							m_logger.warning("SSL connection received. No https contxt created");
				...
				for (String k : headers.keySet()) {
					if (!isValidHeaderKey(k)) {
						reject(Code.HTTP_BAD_REQUEST, requestLine, "Header key contains illegal characters");
						return;
					}
				}
				...
				if (headers.containsKey("Content-Length")
					...
					reject(Code.HTTP_BAD_REQUEST, requestLine, "Conflicting or malformed headers detected");
					return;
				}
				long clen = 0L;
				...
				if (headerValue != null) {
					if (headerValue.equalsIgnoreCase("chunked") && teValueList.size() == 1) {
						...
						reject(Code.HTTP_NOT_IMPLEMENTED, requestLine, "Unsupported Transfer-Encoding value");
						return;
					}
				} else {
				...
				if (ctx == null) {
					if (ExceptionManager.singleton().hasExceptionHandler()) {
						HttpContext context = new HttpContextImpl(https ? "https" : "http", uri.getPath(), null,
								ServerImpl.this);
						reject(Code.HTTP_NOT_FOUND, requestLine,
								ExceptionManager.singleton().getResult(context, new Exception("No HttpHandler")));
					} else {
				...
				/*
				 * check if client sent an Expect 100 Continue. In that case, need to send an
				 * interim response. In future API may be modified to allow app to be involved
				 * in this process.
				 */
				...
				/*
				 * uf is the list of filters seen/set by the user. sf is the list of filters
				 * established internally and which are not visible to the user. uc and sc are
				 * the corresponding Filter.Chains. They are linked together by a LinkHandler so
				...
				m_logger.log(Level.FINER, "ServerImpl.Exchange (2)", ex);
				reject(Code.HTTP_BAD_REQUEST, requestLine, ex.getLocalizedMessage());
			}
		}
		...
		void sendReply(int code, boolean closeNow, String text) {
			try {
				...
				m_logger.log(Level.FINER, "ServerImpl.sendReply", e);
				closeConnection(connection);
			}
		}
		...
		public void run() {
			...
			synchronized (reqConnections) {
				if (MAX_REQ_TIME != -1) {
					...
					for (HttpConnection c : toClose) {
						m_logger.log(Level.FINE, "closing: no request: " + c);
						reqConnections.remove(c);
						allConnections.remove(c);
						c.close();
					}
			...
			synchronized (rspConnections) {
				if (MAX_RSP_TIME != -1) {
					...
					for (HttpConnection c : toClose) {
						m_logger.log(Level.FINE, "closing: no response: " + c);
						rspConnections.remove(c);
						allConnections.remove(c);
						c.close();
					}
